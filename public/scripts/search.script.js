...
const getSearchResults = throttleSearch(async text => {
    ...
    let API_URL = `http://valuta-hub.me/api/v1/search/${text}`
    if(text.startsWith('#')) {
        text = text.replaceAll("#", "")
        API_URL = `http://valuta-hub.me/api/v1/search/trends/${text}`
    }
        ...
        res.forEach(result => {
            ...
            if("username" in result) {
                ...
                if(result.avatar.includes("http") || result.avatar.includes("https")) {
                    ...
                    avatar = `<img src="http://valuta-hub.me/uploads/${result.avatar}" alt="user profile picture">`;
                }
                html = `
                <a href="http://valuta-hub.me/profile/${result.username}">
                ...
                html = `
                <a href="http://valuta-hub.me/?trend=${result.name}">
