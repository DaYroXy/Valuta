function loadMessages(uid) {
    ...
    fetch(`http://valuta-hub.me/api/v1/messages/get/${uid}`)
    .then(res => res.json())
    .then(res => {
        console.log(res)
        if(res.length === 0) {
...
function updateChatInfo(user) {
    ...
    if(user.avatar.includes("http") || user.avatar.includes("https")) {
        ...
        userAvatar.src = `http://valuta-hub.me/${user.avatar}`;
    }
...
function getMessagesList() {
    fetch("http://valuta-hub.me/api/v1/messages/get")
    .then(res => res.json())
    .then(res => {
        if(res.length === 0) {
...
async function updateCurrentChat(userId) {
    fetch(`http://valuta-hub.me/api/v1/messages/get/${userId}`)
    .then(res => res.json())
    .then(res => {
